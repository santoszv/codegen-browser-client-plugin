...
class ResourceModel(private val classInfo: ClassInfo) {
    ...
    private fun generateInit(writer: BufferedWriter) {
        writer.newLine()
        writer.newLine()
        writer.write("    suspend fun initAntiReplayToken() {")
        writer.newLine()
        writer.write("        val xhr = org.w3c.xhr.XMLHttpRequest()")
        writer.newLine()
        writer.write("        xhr.open(\"HEAD\", baseUrl)")
        writer.newLine()
        writer.write("        xhr.setRequestHeader(mx.com.inftel.codegen.ANTI_REPLAY_TOKEN_HEADER, mx.com.inftel.codegen.NULL_TOKEN)")
        writer.newLine()
